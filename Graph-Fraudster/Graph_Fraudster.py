...
parser.add_argument('--datasets', type=str, default='polblogs', help='Dataset to train, citeseer')
...
def train(XA, AA, XB, AB):
    ...
    for epoch in range(args.epoches):
        ...
        if epoch == args.epoches-1:
            ...
            save_path = 'save_model/main/' + str(args.model) + '/' + str(args.datasets) + str(args.seed)

            # 创建路径（如果路径不存在）
            os.makedirs(save_path, exist_ok=True)

            # 保存模型
            torch.save(models['server'], save_path + '/server.pkl')
            torch.save(models['client_A'], save_path + '/A.pkl')
            torch.save(models['client_B'], save_path + '/B.pkl')
...
def evasion_attack(XA, AA, XB, AB, target_node):#这段代码定义了一个 规避攻击（Evasion Attack） 的函数 evasion_attack，其主要作用是测试在经过攻击后的模型（通常是通过加入噪声或其他扰动来测试模型的鲁棒性）是否能够有效逃避检测或避免误分类。与之前的 model_test 函数类似，这个函数会加载训练好的模型，并在给定的目标节点上评估预测结果。
    rebuild_model = {
...
def model_test(XA, AA, XB, AB, target_node): #这段代码定义了一个测试函数 model_test，用于在训练后评估模型的性能，具体是评估通过 垂直联邦学习（VFL） 训练得到的模型，包含两个客户端模型（A 和 B）以及一个服务器模型（S）。
    rebuild_model = {
        'A': torch.load('save_model/main/'+str(args.model)+'/'+str(args.datasets)+str(args.seed)+'/A.pkl'), 'B': torch.load('save_model/main/'+str(args.model)+'/'+str(args.datasets)+str(args.seed)+'/B.pkl'),
        'S': torch.load('save_model/main/'+str(args.model)+'/'+str(args.datasets)+str(args.seed)+'/server.pkl')
    }
...
def surrogate_server_train(emb_infer, emb_B, pred_S):#这段代码定义了一个训练函数 surrogate_server_train，用于训练 代理服务器模型（surrogate_server）。该模型通过输入的节点嵌入（emb_infer 和 emb_B）进行训练，目标是通过最小化 均方误差（MSE） 来调整模型的参数。
    ...
    # 保存模型
    torch.save(surrogate_server, save_path + '/surrogate_server.pkl')


def fgsm_attack(emb_infer, emb_B, surrogate_server, label_B, target_node, eps=0.5):#FGSM，用于生成对抗攻击实例
    cat_emb = torch.cat((emb_infer, emb_B), dim=-1)
    cat_emb = cat_emb.cpu().detach()
    cat_emb = cat_emb.to(device)
...
def inverse_attack(model_B, emb_infer_with_noise, target_node, A_B, X_B): #这段代码实现了 逆向攻击（Inverse Attack），目的是在图神经网络（GNN）中通过对图的邻接矩阵（A_B）进行微小的修改来生成对抗边（adversarial edges）。通过这些修改，可以影响模型的预测，从而达到对抗攻击的效果。具体来说，这段代码在利用图嵌入（emb_infer_with_noise）对目标节点进行攻击时，尝试修改图的结构（通过调整邻接矩阵）来生成对抗边。
    t = target_node[0]
...
def train_surrogate(A, X, labels_B):#这段代码实现了一个训练过程，用于训练一个图神经网络（GNN）的 代理模型（surrogate model）。代理模型是对原始模型的一种近似，用于模拟对抗攻击或者其他研究目的。具体来说，这个函数训练了一个基于图嵌入的分类模型，利用图的邻接矩阵和节点特征来进行节点分类。
...
if __name__ == '__main__':
    ...
    # 保存模型
    torch.save(emb_A, save_path + '/emb_A.pth')
    torch.save(emb_B, save_path + '/emb_B.pth')
    torch.save(labels_B, save_path + '/labels_B.pth')
    # stealing embedding
    ...
    # 定义文件路径
    file_path = 'save_model/main/' + str(args.model) + '/' + str(args.datasets) + str(args.seed) + '/gen_emb_A.pth'

    # 如果路径不存在，创建路径
    os.makedirs(os.path.dirname(file_path), exist_ok=True)

    # 保存模型
    torch.save(gen_emb_A, file_path)
